...
export default function ENSSearchScreen({ onPurchase }: Props) {
  ...
  // Debounced ENS availability search
  useEffect(() => {
  ...
  const handleExecuteRegistration = async () => {
    ...
    setTxProgress({
      stage: 'simulating',
      detail: 'Verifying gas sponsorship policy...',
    })
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <View style={styles.stagesContainer}>
              {[
                { key: 'simulating', label: '1. Gas Sponsorship Verification' },
                { key: 'broadcasting', label: '2. Mempool Broadcast' },
                { key: 'confirming', label: '3. ENS Node Minting' },
              ].map((step, idx) => {
                const isPassed =
